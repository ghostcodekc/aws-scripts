    ...
    for region_name in all_regions:
      ...
      for i in instances.all():
          ...
          for v in vols:
              ...
              print('volname value is {0}'.format(volname))
              print('name variable is now {}'.format(name))
              print('creating tags')
              snapshot.create_tags(  # Add the following tags to the new snapshot
                  Tags = [
                      ...
                      },
                      {
                       ...
                       {
                          'Key': 'Name',
                          'Value': f"{name} autosnap {volname}" #'{0} autosnap {1}'.format(name, volname)
                      }
                  ]
              )
              f"Snapshot will be named: {name} autosnap {volname}"
              f"Snapshont created from {v.id}"
              print('volname value is {0}'.format(volname))
              ...
              for snap in snapshots:
                  ...
                  if created_on <= deletion_date and can_delete == True:
                      print('Snapshot id {0}, ({1}) from {2} is {3} or more days old... deleting'.format(snap.id, snapname, created_on_string, delete_after_days))
